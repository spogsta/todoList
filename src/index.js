//import { compareAsc, format } from 'date-fns'//at some point revist, need better styles and functions imported from date-fn
...
newTodo(
  ...
  "12/12/1212",
  "12:00",
  "play that funky music whiiiteee",
  true
);
...
function displayTodoList() {
  ...
  for (i = 0; i < projectList[currentProject].length; i++) {
    ...
    tr.id = "tableRow" + i;
    tdTitle.appendChild(
      document.createTextNode(projectList[currentProject][i].title)
    );
    tdDate.appendChild(
      document.createTextNode(projectList[currentProject][i].date)
    );
    tdTime.appendChild(
      document.createTextNode(projectList[currentProject][i].time)
    );
    tdDescription.appendChild(
      document.createTextNode(projectList[currentProject][i].description)
    );
    tdComplete.appendChild(
      document.createTextNode(projectList[currentProject][i].complete)
    );
...
function storeTodo() {
  ...
  let storeProjectList = projectList;
  localStorage["storeProjectList"] = JSON.stringify(storeProjectList);
}
let retrievedTodoList = []; //no saved data on load, goofy but works
function retrieveTodo() {
  retrievedTodoList = JSON.parse(localStorage["storeTodoList"]);
  projectList[currentProject] = retrievedTodoList;
  let retrievedProjectList = JSON.parse(localStorage["storeProjectList"]);
...
//next time use var? That would stop needing this?
window.formSubmit = formSubmit;
window.newProject = newProject;
window.selectProject = selectProject;
window.storeTodo = storeTodo;
window.retrieveTodo = retrieveTodo;
